# Data Structures and Algorithms (DSA) 


### Directory Structure
```

codeforces/
│
├── TLE1000/
|    ├── 1155A.cpp      (Reverse a Substring)
|    ├── 1312B.cpp      (Bogosort)
|    ├── 1362B.cpp      (Johnny & Ancient Computer)
|    ├── 1374C.cpp      (Move Brackets)
|    ├── 1411B.cpp      (Fair Numbers)
|    ├── 1418A.cpp      (Buying Torches)
|    ├── 1438B.cpp      (Valerii Against Everyone)
|    ├── 1447B.cpp      (Numbers Box)
|    ├── 1474B.cpp      (Different Divisors)
|    ├── 1485B.cpp      (Add and Divide)
|    ├── 1506C.cpp      (Double-ended Strings)
|    ├── 1567B.cpp      (MEXor Mixup)
|    ├── 1614B.cpp      (Divan and a New Project)
|    ├── 1620B.cpp      (Triangles on a Rectangle)
|    ├── 1632B.cpp      (Roof Construction)
|    ├── 1659A.cpp      (Red Versus Blue)
|    ├── 1691B.cpp      (Shoe Shuffling)
|    ├── 1690D.cpp      (Black and White Stripe)
|    ├── 1704B.cpp      (Luke is a Foodie)
|    ├── 1715B.cpp      (Beautiful Array)
|    ├── 1725B.cpp      (Basketball Together)
|    ├── 1744C.cpp      (Traffic Light)
|    ├── 1765M.cpp      (Minimum LCM)
|    ├── 1791D.cpp      (Distinct Split)
|    ├── 1831B.cpp      (Array merging)
|    ├── 1840C.cpp      (Ski Resort)
|    ├── 1849B.cpp      (Monsters)
|    ├── 1859B.cpp      (Olya and Game with Arrays)
|    ├── 1876A.cpp      (Helmets in Night Light)
|    ├── 1883C.cpp      (Raspberries)
│    └── 1913B.cpp      (Swap and Delete)
│
|   
├── TLE1100/
|
|
|
```
---

> This is contribute by TheValour ✨